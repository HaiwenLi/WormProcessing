function GroupExtractBasedTracking_V1(Folder,tracking_index,FluoType,frame_seq,track_mode,params,stage_motion)
...
% Tracking Parameters
offset_thre = params(1); % 130
neuron_radius = params(2); % 5
intensity_ratio = params(3); % 0.3
I_ratio_thre = params(4);%0.1
start_frame = params(5);
...
if strcmp(FluoType,'red')
    ...
    if nargin == 7 %if porviding stage position
...
neuron_pos(1:size(Int_pos,1),:) = Int_pos(:,:);
...
for i =1:neuron_num
    [neuron_pos_last(i,1),neuron_pos_last(i,2)] = UpdateNeuronPos(Int_pos(i,1),Int_pos(i,2),neuron_radius,intensity_ratio,worm_image);
end
...
if strcmp(track_mode,'update')==1
    for i = 1:neuron_num
        ...
        updated_neuron_pos = load(output_name); % load original neuron positions
        
        len = Tracking_Length/neuron_num; % successful tracking length
        updated_neuron_pos(frame_seq(1:len)-start_frame+1,:) = neuron_pos(neuron_num*((1:len)-1)+i,:);
...
elseif strcmp(track_mode,'create')==1
    for i = 1:neuron_num
        len = Tracking_Length/neuron_num;  % successful tracking length
        xpos = neuron_pos(neuron_num*((1:len)-1)+i,1);
        ypos = neuron_pos(neuron_num*((1:len)-1)+i,2);
        write_neuronpos(posFolder,tracking_index(i),xpos,ypos,'a');
    end
end
