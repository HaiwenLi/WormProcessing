function SingleExtractBasedTracking(frame_list,initial_pos,imgFolder,anchor_filename,anchor_start,output_name,FlouType) 
% Track single neuron
% Input:
...
%     offset_dist = sqrt(sum((anchor_pos(n+anchor_offset,:)-anchor_pos(n+anchor_offset-1,:)).^2));
    ...
    extracted_neurons = load([imgFolder,'Neuron\' WormImages(frame_index).name '.mat']);
    ...
    for i = 1:extracted_num
        [extracted_pos(i,1),extracted_pos(i,2)] = update_center(extracted_pos(i,1),extracted_pos(i,2),search_interval-6,intensity_ratio,Wimage);
    ...
    extracted_index = find(directions>0 & abs(anchor_dist-last_anchor_dist)<anchor_dist_thre & anchor_dist>search_interval-4 & neuron_offset<offset_thre & neuron_I>I_thre);
    
    if ~isempty(extracted_index)
        extracted_index = extracted_index(anchor_dist(extracted_index) == min(anchor_dist(extracted_index)));
        neuron_pos(n,:) = extracted_pos(extracted_index,:);
        ...
        search_interval_local = search_interval*0.4;
...
%         end
    ...
    disp(['neuron pos ' num2str(frame_index) '  Ax = ',num2str(neuron_pos(n,1)) ' Ay = ',num2str(neuron_pos(n,2)),'    anchor_dist = ',num2str(current_anchor_dist)]);
end
